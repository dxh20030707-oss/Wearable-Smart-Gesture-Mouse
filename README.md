...
**一个用 6 个 GPIO 跑通「空中飞鼠 + 电容触控板 + 旋钮滚轮」三合一交互的可穿戴 HID 外设 —— 从驱动寄存器、手势状态机到手机端调参 App 的全栈自研实现。**
...
| **代码量** | 固件 ≈1290 行 C/C++（12 个文件，三层模块化）+ 手机端 ≈790 行 Vue/JS |
...
> 📸 目前仓库尚未包含演示素材。建议补充以下文件后取消本节注释，面试官/访客的首次浏览体验会显著提升：
>
> ```markdown
...
> 本节是项目的工程核心：每个难点都是实际调试中暴露的问题，方案与效果均有代码依据。
...
| 环节 | 参数 | 设计意图 |
| :--- | :--- | :--- |
| BLE HID 上报窗口 | **15ms**（主循环） | 保证 ≥60Hz 报告率，同时为 NimBLE 后台线程让出 CPU |
...
| 陀螺仪死区 | 80 LSB ≈ 1.2 °/s（±500°/s 量程 @65.5 LSB/(°/s)） | 低于人体生理性手抖幅度，静置不漂移 |
| 低通滤波 | 一阶 EMA，α = 0.4 | 兼顾平滑度与相位滞后（不产生「拉丝」延迟感） |
...
| 服务 / 特征值 UUID | `12345678-1234-5678-1234-56789abcdef0` / `…def1` |
